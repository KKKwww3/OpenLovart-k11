    ...
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="flex items-center justify-end gap-4 p-4 text-gray-400">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="hover:text-gray-600 transition-colors ml-2"
          >
            <X size={18} />
          </button>
        )}
            ...
            <div className="space-y-4 mb-6">
              {suggestions.map((item, index) => (
                ...
                >
                  ...
                  <div
                    className={`w-16 h-20 rounded-lg shadow-sm ${item.imageColor} transform group-hover:scale-105 transition-transform rotate-3`}
                  ></div>
                  <div
                    className={`absolute right-8 w-16 h-20 rounded-lg shadow-sm ${item.imageColor} opacity-50 transform rotate-12 -z-10`}
                  ></div>
                </div>
              ))}
            </div>

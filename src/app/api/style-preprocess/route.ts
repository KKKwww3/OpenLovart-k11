...
export async function POST(request: NextRequest) {
  try {
    ...
    const { sceneImageUrl, similarity, keepItems, changeItems, modelId } =
      await request.json();

    if (!sceneImageUrl || typeof sceneImageUrl !== "string") {
      return NextResponse.json(
        { error: "sceneImageUrl is required" },
        { status: 400 },
      );
    }
    ...
    const edgeUrl = `${EDGE_FUNCTIONS_BASE_URL || SUPABASE_URL}/functions/v1/style-preprocess`;
    ...
    console.error("Error preprocessing style:", error);
